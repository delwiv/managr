...
export const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case _loadContacts.succeeded: {
      const { count: previousCount, lazyLoad } = state
      const { count } = action.payload
      const contacts =
        count === previousCount && lazyLoad ? [...state.contacts, ...action.payload.contacts] : action.payload.contacts
      console.log({ previousCount, count, lazyLoad, l: contacts.length })
      return {
        ...state,
